...
| ID Analytics personalizzato (`s.visitorID`) | L&#39;impostazione di un ID cliente tramite `s.visitorID` è un metodo di identificazione degli utenti in Adobe Analytics. Tuttavia, le integrazioni in cui vengono esportati o importati i dati di [!DNL Analytics] tramite il servizio ID visitatore non funzionano quando un visitatore viene identificato tramite `s.visitorID.`<br>Sono inclusi, ma senza limitazioni, tipi di pubblico condivisi, [!DNL Analytics] per Adobe Target (A4T) e [!DNL Customer Attributes].<br>Per queste integrazioni, l&#39;impostazione di un ID Analytics personalizzato non è supportata. |
...
| È richiesta la migrazione al servizio ID visitatore? | La migrazione dipende dalle applicazioni che utilizzi. <ul><li>Adobe Analytics: fortemente consigliata </li><li>Adobe Target: obbligatoria. </li></ul>L’utilizzo del servizio ID visitatore consente di utilizzare le funzionalità più recenti di CX Enterprise, tra cui i tipi di pubblico in tempo reale, la modernizzazione di Adobe Target, l’integrazione di Analytics e il tracciamento heartbeat nei video. |
